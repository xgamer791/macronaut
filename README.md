...
- **Profile page** — banner and profile picture you upload yourself, name, handle, bio, sport and location, your home gym, followers / following / posts, a photo wall, fitness groups, and posts; private by default, and one toggle away from a shareable public page at `/u/<handle>` that other accounts can follow. Reached from the avatar in the Today header
- **Home gym** — the last step of onboarding (and Settings → Home gym): find your gym by name within seven miles of your location or a typed address, powered by Google Places on the server, and land in the one group everybody at that gym shares. The first claim creates it; nobody owns it. Members see each other's public profiles, and keep the group in shape by vote — three in a week suspend someone, five remove them
- **Groups** — create and join fitness groups, local-first discovery, and the gym groups above; every group has one chat for its members, with photos and clips, unread counts, and a single bell row per group
