/** Shared Grok vision helpers. Used by the Convex food-scan action so the
 * xAI key never leaves the backend. */
...
export const GROK_FOOD_PROMPT = `You are a nutrition expert helping log a meal from a photo.
...
export function parseEstimate(raw: unknown): GrokFoodEstimate {
  ...
  const nutrition: GrokFoodEstimate['nutrition'] = { calories: Math.round(calories) };
...
/** Analyze a food photo with an xAI key. Callers must never send the key to a client. */
export async function analyzeFoodPhoto(opts: {
  ...
  if (!key) throw new Error('AI food scan is not configured');
        ...
        input: [
          {
            ...
            content: [
              ...
              { type: 'input_text', text: GROK_FOOD_PROMPT },
            ],
          },
        ],
      }),
      ...
      messages: [
        {
          ...
          content: [
            ...
            { type: 'text', text: GROK_FOOD_PROMPT },
          ],
        },
      ],
    }),
...
function parseApiError(status: number, body: string): string {
  if (status === 401 || status === 403) return 'Grok API key was rejected';
