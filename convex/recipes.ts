...
function toRecipe(doc: Doc<'recipes'>) {
...
export const create = mutation({
  ...
  handler: async (ctx, input) => {
    ...
    const doc = {
      ...
      favorite: false,
      deleted: false,
      ingredients: buildItems(input.items),
      createdAt: now,
      updatedAt: now,
    };
    const id = await ctx.db.insert('recipes', doc);
    return toRecipe({ _id: id, _creationTime: Date.now(), ...doc });
  },
});
...
export const update = mutation({
  ...
  handler: async (ctx, { id, patch }) => {
    const userId = await requireUserId(ctx);
    const existing = await requireOwned(ctx, 'recipes', id, userId);
    ...
    const next = {
      ...
      ingredients: patch.items ? buildItems(patch.items) : existing.ingredients,
      updatedAt: nowIso(),
    };
    const { _id, _creationTime: _t, ...fields } = next;
    await ctx.db.replace(_id, fields);
    return toRecipe(next);
  },
});
...
export const remove = mutation({
  ...
  handler: async (ctx, { id }) => {
    const userId = await requireUserId(ctx);
    await requireOwned(ctx, 'recipes', id, userId);
...
export const duplicate = mutation({
  ...
  handler: async (ctx, { id }) => {
    ...
    const doc = {
      ...
      favorite: false,
      deleted: false,
      ingredients: buildItems(existing.ingredients),
      createdAt: now,
      updatedAt: now,
    };
    const newDocId = await ctx.db.insert('recipes', doc);
    return toRecipe({ _id: newDocId, _creationTime: Date.now(), ...doc });
  },
});

export const setFavorite = mutation({
  args: { id: v.id('recipes'), favorite: v.boolean() },
