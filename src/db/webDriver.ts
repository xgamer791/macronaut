 ...
 *
 * Each account stores its bytes under its own record key so signing in as a
 * different user in the same browser profile cannot read the previous
 * account's database. */

const IDB_NAME = 'macronaut';
const IDB_STORE = 'sqlite';
const DEFAULT_IDB_KEY = 'main';
...
export async function deleteWebDatabase(key: string = DEFAULT_IDB_KEY): Promise<void> {
  const idb = await openIdb();
...
export async function createWebDriver(idbKey: string = DEFAULT_IDB_KEY): Promise<Database> {
  ...
  const existing = await idbLoad(idbKey).catch(() => null);
  ...
  function scheduleSave() {
    ...
    saveTimer = setTimeout(() => {
      idbSave(idbKey, db.export()).catch((err) => {
